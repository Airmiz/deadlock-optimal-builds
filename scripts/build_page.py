...
<script>
  ...
  // category at the END of each phase (early/mid/late).
  function computeSpikeProgress(itemsBySlice) {
    ...
    for (const it of sorted) {
      // bucket: track cumulative as items roll in
      if (it.category in totals) totals[it.category] += (it.cost || 0);
      // when the next item is in a later phase, snapshot
    ...
    byPhaseEnd[lastPhase] = { ...totals };
    // Fill any gaps (snapshot earlier phases if no items existed in them)
    let last = { weapon: 0, vitality: 0, spirit: 0 };
    for (const ph of ['early','mid','late']) {
      if (byPhaseEnd[ph] === null) byPhaseEnd[ph] = { ...last };
      last = byPhaseEnd[ph];
    }
